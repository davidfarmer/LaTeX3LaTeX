...
def lmfdb(text):
    ...
    the_lines = thetext.split("\n")

    the_answer = "degree3conductor4a = {\n"
    ...
    for line in the_lines:
        ...
        if line.startswith("origin"):
            print("found the parameter names")
        elif line.startswith("text"):
            print("found the parameter types")
            parameter_types = line.split("|")
        else:
            this_line_converted = "{"
            this_data = line.split("|")
#           print("found data:", len(this_data))
            for index, item in enumerate(this_data):
               if item == "\\N":
                   item = '"' + "\\\\N" + '"'
                   # can happen for jsonb text boolean numeric and other
               elif parameter_types[index] in ["numeric", "smallint", "double precision"]:
                   pass
               elif parameter_types[index] == "bigint":
                   print("bigint", item)
                   item = '"' + item + '"'
               elif parameter_types[index] == "text":
                   item = '"' + item + '"'
               elif parameter_types[index] == "boolean":
                   item = '"' + item + '"'  # t or f
               elif parameter_types[index] == "jsonb":
                   if not item.startswith("["):
                       print("error: expected list", item)
                   item = re.sub('"', '', item)
                   item = re.sub('\(', '', item)
                   item = re.sub('\)', '', item)
                   item = re.sub("\[", "{", item)
                   ...
                   print(parameter_types[index], item)

               this_line_converted += item
               this_line_converted += ", "
               

            this_line_converted = this_line_converted[:-2]   # delete ", "
            this_line_converted += "}"

 #       print("this_line_converted", this_line_converted)
            the_answer += this_line_converted        
            the_answer += ","
            the_answer += "\n"

    the_answer = the_answer[:-2]  # delete final ",\n"
    the_answer += "\n" + "}"
